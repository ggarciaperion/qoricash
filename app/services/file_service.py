...
class FileService:
    ...
    def _configure_cloudinary(self):
        ...
        try:
            ...
            if not cloud_name or cloud_name == 'your-cloud-name':
                logger.error("ERROR: CLOUDINARY_CLOUD_NAME no está configurado correctamente en .env")
                self.configured = False
                return

            if not api_key or api_key == 'your-api-key':
                logger.error("ERROR: CLOUDINARY_API_KEY no está configurado correctamente en .env")
                self.configured = False
                return

            if not api_secret or api_secret == 'your-api-secret':
                logger.error("ERROR: CLOUDINARY_API_SECRET no está configurado correctamente en .env")
                self.configured = False
                return
            ...
            logger.info(f"OK Cloudinary configurado correctamente: {cloud_name}")
        except Exception as e:
            logger.error(f"ERROR Error configurando Cloudinary: {e}")
            self.configured = False
    ...
    def upload_file(self, file, folder, public_id_prefix=None):
        ...
        try:
            ...
            # public_id SIN extensión dentro del folder (evita duplicación de carpeta y extensión)
            if public_id_prefix:
